var MapModule = function(view, zoom, map_width, map_height) {
  ...
  // Create Leaflet map and Agent and Patch layers
  var mymap = L.map('mapid').setView(view, zoom);
  var AgentLayer = L.geoJSON().addTo(mymap);
  var PatchLayer = L.geoJSON().addTo(mymap);

  // create the tile layer with correct attribution
	var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
	var osmAttrib='Map data © <a href="http://openstreetmap.org">OpenStreetMap</a> contributors';
	var osm = new L.TileLayer(osmUrl, {minZoom: 3, maxZoom: 14, attribution: osmAttrib});
  mymap.addLayer(osm);
